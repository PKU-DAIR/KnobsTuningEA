...
class TurboM(Turbo1):
    ...
    def save_parameter(self):
        dump_dir = {}
        Y_variableL = self.fX.flatten().tolist()
        for i in range(0, len(Y_variableL)):
            if Y_variableL[i] >= 0 and self.y_variable == 'tps':
                Y_variableL[i] = max([i for i in Y_variableL if i>0]) * 0.9
            if Y_variableL[i] <= 0 and self.y_variable == 'lat':
                Y_variableL[i] = max([i for i in Y_variableL if i < 0]) * 1.1
        self.fX = np.array(Y_variableL).reshape(self.fX.shape)
        dump_dir['X'] = self.X
        dump_dir['fX'] = self.fX
        dump_dir['_idx'] = self._idx
        dump_dir['failcount'] = self.failcount
        dump_dir['succcount'] = self.succcount
        dump_dir['length'] = self.length
        dump_dir['n_evals'] = self.n_evals
        f = open(self.save_file, 'wb')
        pickle.dump(dump_dir, f)
        f.close()

    def load_paremeter(self):
        f = open(self.save_file, 'rb')
        dump_dir = pickle.load(f)
        self.X = dump_dir['X']
        self.fX = dump_dir['fX']
        self._idx = dump_dir['_idx']
        self.failcount = dump_dir['failcount']
        self.succcount = dump_dir['succcount']
        self.length = dump_dir['length']
        self.n_evals = dump_dir['n_evals']
